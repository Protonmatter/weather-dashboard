  ...
  return (
    <Card title="Wind & Visibility" className="fadein h-full">
      <dl className="grid flex-1 grid-cols-2 items-center gap-3">
        ...
        <div><dt className="text-[10px] font-semibold uppercase tracking-wider text-white/50">Visibility</dt><dd className={`mt-1 font-light tabular-nums ${current.visibility === null ? "text-base" : "text-3xl"}`}>{current.visibility === null ? "Unavailable" : <>{current.visibility.toFixed(1)}<span className="ml-1 text-sm text-white/55">mi</span></>}</dd></div>
      </dl>
...
export function TemperatureTrendCard({ daily, T }: { daily: readonly DayPoint[]; T: Convert }) {
  ...
  // Preserve the existing 10..290 by 20..88 drawing area via one tested geometry path.
  const geometry = buildTrendGeometry(values, 300, 88, 10);
  const { min, max } = geometry;
  const points = geometry.points.map(({ x, y }) => `${x},${y + 10}`).join(" ");
  return (
