      ...
      <dl className="grid flex-1 grid-cols-2 items-center gap-3">
        <div>
          ...
          <dd className="mt-1 text-3xl font-light tabular-nums">
            ...
            <span className="ml-1 text-sm text-white/55">mph</span>
          </dd>
        </div>
        <div>
          <dt className="text-[10px] font-semibold uppercase tracking-wider text-white/50">
          ...
          <dd className="mt-1 text-3xl font-light tabular-nums">
            ...
            <span className="ml-1 text-sm text-white/55">mi</span>
          </dd>
        </div>
      </dl>
      <div className="glass-inset mt-4 h-2 overflow-hidden rounded-full" aria-hidden="true">
        <div
...
export function TemperatureTrendCard({
  ...
  if (days.length < 2) {
    return (
      <Card title="Temperature Trend" className="h-full" data-testid="temperature-trend-card">
  ...
  const points = values
    .map((value, index) => `${10 + index * (280 / (values.length - 1))},${88 - ((value - min) / span) * 68}`)
    .join(" ");

  return (
    <Card title="Temperature Trend" className="fadein h-full" data-testid="temperature-trend-card">
      <div className="flex items-baseline justify-between gap-3">
        <span className="text-3xl font-light tabular-nums">{values.at(-1)}°</span>
        <span className="text-[10px] text-white/50">{min}–{max}° midpoint</span>
      </div>
      <svg
        viewBox="0 0 300 100"
        className="mt-2 h-28 w-full"
        role="img"
        aria-label={`Seven-day temperature trend from ${values[0]} to ${values.at(-1)} degrees`}
      >
        <polyline
          points={points}
          fill="none"
        ...
        {points.split(" ").map((point) => {
          ...
          return <circle key={point} cx={cx} cy={cy} r="2.4" fill="var(--accent-cyan)" />;
        })}
      </svg>
      <div className="grid grid-cols-7 text-center text-[9px] text-white/45" aria-hidden="true">
        {days.map((day) => (
